...
export default class Bard {
  ...
  constructor(
    ...
    private proxies?: { [key: string]: string }
  ) {
  ...
  private async getSNlM0e(): Promise<string> {
    ...
    try {
      const resp: AxiosResponse<string> = await this.session.get(
        'https://bard.google.com/'
      );
      if (resp.status !== 200) {
        throw new Error(
          `Response code not 200. Response Status is ${resp.status}`
        );
      }
      ...
      throw new Error(
        `Error occurred while fetching SNlM0e value: ${
  ...
  public async getAnswer(inputText: string): Promise<object> {
    ...
    try {
      ...
      throw new Error(
        `Error occurred while fetching Bard answer: ${(error as Error).message}`
      );
    }
  }
}
